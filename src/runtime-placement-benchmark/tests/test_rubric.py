...
# v0.2: dimension count 7 → 8 (added D8 per Codex Loop #4 H14). All-dimension score dicts
# now include D8.
_ALL_DIMS = ("D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8")
...
def test_default_rubric_uses_default_weights() -> None:
    ...
    assert r.weights["D8"] == 4  # v0.2: D8 evidence integrity weight=4
...
def test_rubric_rejects_unknown_weight() -> None:
    # v0.2: D8 is now known; use D99 as the definitely-unknown probe
    weights = dict.fromkeys((*_ALL_DIMS, "D99"), 3)
    with pytest.raises(ValueError, match="unknown dimension"):
        WeightedScoreRubric(weights=weights)


def test_weighted_score_matches_expected_math() -> None:
    scores = dict.fromkeys(_ALL_DIMS, 5)
    r = WeightedScoreRubric()
...
def test_weighted_score_rejects_out_of_range() -> None:
    scores = dict.fromkeys(_ALL_DIMS, 3)
    scores["D3"] = 10
