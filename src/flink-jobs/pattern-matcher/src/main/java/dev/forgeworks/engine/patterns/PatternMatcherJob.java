 ...
 *
 * <p>Pipeline: Kafka (forge.insights.realtime) → Deserialize → KeyBy (group key: source +
 * repo/namespace) → PatternWindowFunction (stateful, 10-min sliding window) → Kafka
 * (forge.jobs.pending)
 *
 * <p>Where it sits in the pipeline: Webhook → Gateway → Kafka → Event Router →
 * forge.insights.realtime ↓ Pattern Matcher (this job) ↓ forge.jobs.pending
 *
 * <p>MVP Rules: PAT-DEPLOY-001: Rapid successive deployments (>3 ArgoCD syncs in 10 min)
 * PAT-K8S-001: Pod crash loop (>3 restart events in 10 min) PAT-CI-001: PR merged without tests
 * (merge + no check_run in window)
 *
 * <p>Performance target: Pattern detection within 100ms of event arrival.
 */
public class PatternMatcherJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<EventEnvelope> events =
                env.fromSource(source, WatermarkStrategy.noWatermarks(), "kafka-insights")
        ...
        KafkaSink<PatternAlert> alertSink =
                KafkaSink.<PatternAlert>builder()
                        .setBootstrapServers(kafkaBootstrap)
                        .setRecordSerializer(
