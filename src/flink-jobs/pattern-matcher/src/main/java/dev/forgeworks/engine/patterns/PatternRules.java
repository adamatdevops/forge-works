 ...
 *
 * Each rule evaluates a window of recent events for a given group key
 * (e.g., all events from the same repository or namespace). If the rule
 * condition matches, it produces a PatternAlert.
 *
 * Why rules (not ML) for MVP:
 * - Deterministic — easy to debug and explain
 * - Zero training data needed — works from day one
 * - Fast — pure Java logic, no model loading
 * - Extensible — add new rules without retraining
 *
 * ML-based pattern detection comes in Phase 3 (Airflow training pipeline)
 * and Phase 2 Sprint 2.2 tasks T-E2.2.5/6 (model loader).
 */
public class PatternRules {

    /**
     * Evaluate all rules against a window of events.
     * Returns a list of alerts (may be empty if no patterns matched).
     */
    ...
    /**
     * PATTERN: Rapid Successive Deployments
     * Trigger: >3 ArgoCD sync events within the window for the same app
     * Severity: high
     * Why: Multiple rapid deploys often signal instability — rollback loops,
     *      config thrashing, or an engineer force-pushing fixes repeatedly.
     */
    static PatternAlert checkRapidDeployments(String groupKey, List<EventEnvelope> window) {
        ...
        for (EventEnvelope e : window) {
            if ("argocd".equals(e.getSource()) && e.getType() != null && e.getType().contains("sync")) {
        ...
        if (syncEventIds.size() > 3) {
            return PatternAlert.create(
                    ...
                    10,
                    syncEventIds,
                    new HashMap<String, Object>() {{ put("threshold", 3); put("actual", syncEventIds.size()); }}
            );
        }
        return null;
    }
     ...
     * Trigger: >3 Kubernetes pod restart/crash events within the window
     * Severity: critical
     * Why: CrashLoopBackOff means the pod keeps dying and restarting.
     *      Catching this early prevents cascading failures.
     */
    ...
    /**
     * PATTERN: Merge Without Tests
     * Trigger: GitHub merge event without a corresponding workflow/check event in the window
     * Severity: medium
     * Why: Merging PRs without test runs risks pushing broken code to production.
     *      This pattern catches repos where CI isn't running or was skipped.
     */
    static PatternAlert checkMergeWithoutTests(String groupKey, List<EventEnvelope> window) {
        ...
        if (!mergeEventIds.isEmpty() && !hasTestRun) {
            return PatternAlert.create(
                    ...
                    10,
                    mergeEventIds,
                    new HashMap<String, Object>() {{ put("merge_count", mergeEventIds.size()); put("tests_detected", false); }}
            );
        }
        return null;
    }
