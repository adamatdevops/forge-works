 ...
 *
 * <p>All HTTP calls have a 5-second timeout to prevent blocking Flink processing. Called from a
 * background thread, never from the scoring hot path.
 */
public class MLflowModelAdapter implements Serializable {
    ...
    /**
     * Check MLflow for a newer version of a registered model. Returns a new ScoringModel if
     * updated, null if no change.
     */
    public ScoringModel checkForUpdate(String modelName) {
        try {
            ...
            LOG.info(
                    "New model version detected: {} v{} (was: {})",
                    modelName,
                    latestVersion,
                    currentVersion);
            ...
            if (weights == null || weights.isEmpty()) {
                LOG.warn(
                        "No feature importance found for {} v{}, skipping",
                        modelName,
                        latestVersion);
                return null;
            }
    ...
    /**
     * Fetch feature importance metrics logged during training. Training logs these as
     * importance_{feature_name} via mlflow.log_metric().
     */
    private Map<String, Double> fetchFeatureImportance(String modelName, String version)
            throws Exception {
        HttpClient client = HttpClient.newBuilder().connectTimeout(HTTP_TIMEOUT).build();
        ...
        HttpResponse<String> versionResp =
                client.send(versionReq, HttpResponse.BodyHandlers.ofString());
        ...
        HttpResponse<String> metricsResp =
