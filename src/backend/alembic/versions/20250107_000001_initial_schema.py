...
def upgrade() -> None:
    ...
    service_status = postgresql.ENUM(
        "healthy", "degraded", "unhealthy", "unknown", "provisioning",
        name="service_status",
        create_type=True,
    )
    ...
    service_tier = postgresql.ENUM(
        "critical", "standard", "internal", "experimental",
        name="service_tier",
        create_type=True,
    )
    ...
    anomaly_type = postgresql.ENUM(
        "high_deploy_frequency", "consecutive_rollbacks", "pipeline_failing",
        "health_degraded", "unusual_error_rate", "resource_spike", "drift_detected",
        name="anomaly_type",
        create_type=True,
    )
    ...
    action_type = postgresql.ENUM(
        ...
        "service_deployed", "service_rolled_back",
        "template_created", "template_updated", "template_deprecated",
        "recommendation_requested", "recommendation_accepted", "recommendation_overridden",
    ...
    op.create_table(
        ...
        sa.Column("template_id", postgresql.UUID(as_uuid=False), sa.ForeignKey("templates.id"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("top_template_id", postgresql.UUID(as_uuid=False), sa.ForeignKey("templates.id"), nullable=False),
        sa.Column("top_score", sa.Float, nullable=False),
        sa.Column("selected_template_id", postgresql.UUID(as_uuid=False), sa.ForeignKey("templates.id"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("team_id", postgresql.UUID(as_uuid=False), sa.ForeignKey("teams.id"), nullable=True),
        sa.Column("service_id", postgresql.UUID(as_uuid=False), sa.ForeignKey("services.id"), nullable=True),
