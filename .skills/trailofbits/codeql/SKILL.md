...
name: codeql
vendor: trailofbits
slug: trailofbits/codeql
source-url: https://github.com/trailofbits/skills/tree/main/plugins/static-analysis/skills/codeql
source-canonical: https://github.com/trailofbits/skills/tree/a56045e9ae00b3506cacefea0f672aab0a1a6e3c/plugins/static-analysis/skills/codeql
...
context-cost-tokens: 3433
owner: adamatdevops
---

<!-- Source: https://github.com/trailofbits/skills/tree/a56045e9ae00b3506cacefea0f672aab0a1a6e3c/plugins/static-analysis/skills/codeql/SKILL.md · SHA: a56045e9ae00b3506cacefea0f672aab0a1a6e3c · Audited: 2026-05-20 · **17th vendored skill — second of THREE simultaneous adoptions from the trailofbits/static-analysis multi-skill plugin bundle (§AN sarif-parsing + §AO codeql + §AP semgrep). See §AN sarif-parsing Notes for full bundle audit + AB-037 candidate-resolution event narrative; this Notes paragraph is the §AO codeql specifics.** **Tier-staleness audit (per new §4.4.3 step 2):** (i) **YES — explicit file-write directives at SKILL.md L37-39 + L44-54 + workflows/build-database.md + workflows/run-analysis.md**: `mkdir -p $OUTPUT_DIR` (creates output directory), database build creates `codeql.db/` + `codeql-database.yml`, scan output to `$OUTPUT_DIR/raw/results.sarif` + `$OUTPUT_DIR/results/results.sarif`, diagnostic queries write CSVs to `diagnostics/`, data extensions write YAMLs to `extensions/`. **Multiple file CREATE operations → Tier 2 per §J R3-A-3.** (ii) `allowed-tools: Bash Read Write Edit Glob Grep AskUserQuestion TaskCreate TaskList TaskUpdate TaskGet TodoRead TodoWrite` — `Bash` + `Write` + `Edit` declared → shell-execute + repo-write Tier 2; (iii) NO `commands/` directory; (iv) NO direct network egress in SKILL.md body (CodeQL CLI fetches query packs from `github.com/codeql` registry under-the-hood — mediated by `codeql pack download` which uses gh CLI auth; not declared as skill-level egress); (v) NO `Task` tool / cross-agent surface. **Final Tier 2 verdict for §AO codeql**: `shell-execute+repo-write` — runs `codeql` CLI via Bash, creates database directory + SARIF outputs + data extensions via mkdir/Write. **Bundle (LARGEST in framework after this adoption):** SKILL.md 3433t + 12 references/*.md files (15871t cumulative — `ruleset-catalog.md` 574t, `quality-assessment.md` 1828t, `build-fixes.md` 739t, `threat-models.md` 665t, `performance-tuning.md` 799t, `diagnostic-query-templates.md` 2585t, `extension-yaml-format.md` 2076t, `language-details.md` 1286t, `macos-arm64e-workaround.md` 1970t, `run-all-suite.md` 1033t, `important-only-suite.md` 1381t, `sarif-processing.md` 835t) + 3 workflows/*.md files (8094t — `build-database.md` 2608t, `create-data-extensions.md` 2550t, `run-analysis.md` 2936t) + LICENSE byte-identical. **Worst-case archive 27,298 tokens** — largest single-skill bundle in framework; far exceeds §3B 5K-preferred cap. Acknowledged per §3B archive-load metering: the 15 reference + workflow companion files are load-on-demand (agent reads them only when the codeql workflow phase requires the specific reference); SKILL.md alone is 3433t (within typical range). **Phase-A/Phase-B gate doctrine (per §4.5.2 step 4 Tier 2 handling):** §AO codeql is overwhelmingly Phase B in practice — the skill's Phase 1 starts with `mkdir -p $OUTPUT_DIR` + `codeql database create` (both Phase B operations); Phase A only applies if the user pre-supplies a built database AND the agent only needs to RUN queries (no database build, no extensions). Most invocations: Phase B from step 1; neutral 3-option confirmation prompt MUST fire. **Sub-cluster within static-analysis plugin:** §AO codeql is the SCAN-EXECUTION skill paired with §AN sarif-parsing (output processing). Canonical chain: `§AO codeql scan → §AN sarif-parsing` (process SARIF output). No internal cross-references to other adopted ToB skills (no §F/§G/§H/§J references in codeql SKILL.md body — codeql is methodology-isolated within the static-analysis sub-cluster). **Cluster-coupling with §F/§G/§H/§J + §AN/§AP (FIRST 7-skill ToB cluster):** §AO shares upstream repo `trailofbits/skills` + SHA `a56045e9...` with all 6 other ToB adoptions; ToB cluster grows 4→7 skills, becomes LARGEST cluster in framework (overtaking hamelsmu 5-skill). **Body sha256:** `40e59ff6a6cdfc28...`. NO `claude -p` subprocess paths, NO bundled subagents (the `agents/semgrep-scanner.md` sub-agent is part of §AP semgrep's bundle, NOT §AO codeql), NO model-client invocations across the 15 companion files (all .md; no .py/.sh files in codeql's bundle). **§D-parallel observation:** §AO codeql + §D anthropics/skill-creator share the "Tier 2 with substantial reference companion" shape. -->
