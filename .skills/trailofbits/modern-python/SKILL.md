...
name: modern-python
vendor: trailofbits
slug: trailofbits/modern-python
source-url: https://officialskills.sh/trailofbits/skills/modern-python
source-canonical: https://github.com/trailofbits/skills/tree/a56045e9ae00b3506cacefea0f672aab0a1a6e3c/plugins/modern-python/skills/modern-python
...
context-cost-tokens: 14789
owner: adamatdevops
---

<!-- Source: https://github.com/trailofbits/skills/tree/a56045e9ae00b3506cacefea0f672aab0a1a6e3c/plugins/modern-python/skills/modern-python/SKILL.md · SHA: a56045e9ae00b3506cacefea0f672aab0a1a6e3c · Audited: 2026-05-17 · Fourth Trail of Bits adoption (vendor concentration now 4/7); same SHA as §F + §G + §H — single supply-chain root for all ToB skills. Upstream SKILL.md declares NO `allowed-tools` frontmatter — observed scope `read-only + transform` (Tier 1 per AGENT_SKILLS.md §2E): skill READS existing config + PROPOSES edits via Edit tool; user reviews per §3.2 advisory-only. NO shell-execute, NO `claude -p` subprocess paths, NO transitive skill loads, NO cross-agent invocations. Embedded bash snippets across SKILL.md + 9 references/*.md are USER-FACING documentation/recommendations (`uv add`, `uv sync`, `prek install`, `uvx cookiecutter gh:...`), NOT skill-executed at runtime. Includes 11 byte-verbatim companion files: 9 reference docs (dependabot.md, migration-checklist.md, pep723-scripts.md, prek.md, pyproject.md, ruff-config.md, security-setup.md, testing.md, uv-commands.md) + 2 inert config templates (dependabot.yml, pre-commit-config.yaml — for user copy-paste; `<latest>` placeholders MUST be replaced before user commits). See AGENTS.md §4.5.3 §J entry + research/agents/evaluation_list.md §J Notes for the full audit. -->
