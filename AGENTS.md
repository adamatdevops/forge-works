...
- **Auto-loading every adopted skill into context at session start.** Loading all 6 currently-adopted skills' SKILL.md content as a block consumes ~14.0K tokens (884 §F + 987 §AF + 2133 §G + 1668 §H + 7252 §D + 1022 §T); auto-loading worst-case read-into-context archives (SKILL.md + all read-by-Claude companion files for multi-file skills) consumes ~40.3K tokens (adds 7549 §G worst-case + 8087 §H worst-case + 18659 §D worst-case read-into-context + 4127 §T worst-case). §D's full archive INCLUDING executable scripts that don't load into Claude's context window is 50312 tokens. Per `AGENT_SKILLS.md` §3B, individual skills > 5K are flagged and > 10K require justification; the full set crosses both thresholds many times over with zero corresponding work performed. Selective invocation per §4.5.1 triggers is the budget-honest path.
