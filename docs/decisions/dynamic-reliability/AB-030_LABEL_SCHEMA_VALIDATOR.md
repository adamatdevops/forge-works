# AB-030 Label Schema Validator Scoping RFC (v0.3 draft) — shared library for `forge.events.ground_truth.v1` conformance

> **Status:** Scoping draft (v0.3 — GT contract amendments landed 2026-07-29 unblocking F3/F4/F8; interim rules lifted to hard-reject). Not yet approved for build. Approval means: the API surface, validation rules, and distribution model are locked and the library may be implemented.
...
- **Warning triggers** — 7 events, one per W1 (with configured threshold), W2, W3, W4 (`derived + uncertain`), W5 (`logic_ref: ftp://…`), W6 (`intervention_ids` count > 10), W7 (no catalog); each passes with the corresponding warning. *(W8 was deleted in v0.3; not tested.)*
...
## 13. v0.3 → v1 acceptance criteria

*(v0.2 filed 3 GT-amendment prerequisites (F3/F4/F8); v0.3 landed those amendments and lifted the interim rules. Remaining criteria below.)*
