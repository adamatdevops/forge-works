# AB-030 Label Schema Validator Scoping RFC (v0.5 draft) — shared library for `forge.events.ground_truth.v1` conformance

> **Status:** Scoping draft (v0.5 — library v0.1.0 shipped 2026-07-29 at `src/label-schema-validator/`; §5.1 package-layout path corrected to match actual monorepo convention). RFC has served its purpose; awaiting scoping-approval to promote to canonical library documentation.
...
*(v0.5 correction — v0.1–v0.4 assumed a non-existent `src/python/` root; actual ForgeWorks monorepo convention is per-service directories at `src/<service>/` each with its own `pyproject.toml` and co-located `tests/`. Path updated to match reality; import path preserved via namespace package.)*

- Import path: `forge_works.dr.label_schema_validator` (preserved; namespace package under `forge_works.dr` allows future sibling DR libraries).
- Repository location: `src/label-schema-validator/forge_works/dr/label_schema_validator/`.
- Test location: `src/label-schema-validator/tests/` (co-located with library, matching `src/webhook-gateway/tests/` precedent).
- Package metadata: `src/label-schema-validator/pyproject.toml` — service-directory pattern.
...
- **2026-07-29 (evening):** v0.5 — library v0.1.0 shipped at `src/label-schema-validator/` implementing every rule in §3.1–§3.5 and every warning in §3.4. 73 tests pass (63 conformance + 10 property/hypothesis). Ruff clean. §5.1 package-layout path corrected — v0.1–v0.4 assumed a non-existent `src/python/` root; actual monorepo convention is per-service directory (`src/<service>/pyproject.toml` + co-located `tests/`, matching `src/webhook-gateway/` precedent). Import path `forge_works.dr.label_schema_validator` preserved via namespace package. AB-028 spike deriver integration remains the only outstanding v1 acceptance item.
