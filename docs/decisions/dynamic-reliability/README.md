...
| [`AB-030_LABEL_SCHEMA_VALIDATOR.md`](AB-030_LABEL_SCHEMA_VALIDATOR.md) | v0.4 draft (all 4 v0.3 → v1 non-GT items resolved 2026-07-29; only AB-028 spike integration remaining) — awaiting scoping-approval | Shared library that enforces `GROUND_TRUTH_INTERVENTION_CONTRACT.md` v0.3 §2.1/§2.2/§3/§5/§6.2 rules on every label event before emission. Blocks AB-028 spike execution per that RFC's §3 Schema-conformance block. |
| [`AB-029_RUNTIME_PLACEMENT_SPIKE.md`](AB-029_RUNTIME_PLACEMENT_SPIKE.md) | v0.1 draft (2026-07-29) — awaiting scoping-approval | Architecture spike comparing 5 runtime-placement options (sibling Flink / pattern-matcher extension / insight-generator extension / batch materialized view / dedicated inference service) across 7 dimensions. Runs in parallel with AB-028; uses AB-028's reference workload. Independent of contract graduation. |
